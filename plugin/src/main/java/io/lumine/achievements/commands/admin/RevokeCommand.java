...
public class RevokeCommand extends Command<MythicAchievementsPlugin> {

    public RevokeCommand(AdminCommand plugin) {
        super(plugin);
    }
    ...
    public boolean onCommand(CommandSender sender, String[] args) {
        ...
        if(!profile.hasCompleted(achieve)) {
            CommandHelper.sendError(sender, "Player has not completed that achievement");
            return true;
        }
        
        profile.revokeAchievement(achieve);
        
        CommandHelper.sendSuccess(sender, "Revoked achievement from player <yellow>" + player.getName());
        return true;
    }
    ...
    public List<String> onTabComplete(CommandSender sender, String[] args) {
        if(args.length == 1) {
            ...
            var profile = getPlugin().getProfiles().getProfile(player);
            return StringUtil.copyPartialMatches(args[1], profile.getCompletedAchievementNames(), new ArrayList<>());
        } else {
            return Collections.emptyList();
        }
    }
